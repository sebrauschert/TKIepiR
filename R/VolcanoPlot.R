...
VolcanoPlot <- function(EWAS, annotate=FALSE, p.column.name = p.column.name, beta.column.name = beta.column.name, title="title"){
  ...
  if (annotate==FALSE){
    EWAS <- as.data.frame(EWAS[,c("ID", p.column.name,beta.column.name)])
    EWAS <- as.data.frame(annotateCpG(EWAS))
  }
